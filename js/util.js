...
function util() {
	if (utilDiv) {
		...
		saveBtn.onclick = function(){saveAFile(prompt('Save Filename',"fname.txt"))};
		var loadBtn = createDiv(container, "console-button", "load");
		loadBtn.onclick = function(){loadAFile(prompt('Load Filename',"fname.txt"))};
		var evalBtn = createDiv(container, "console-button", "eval");
		evalBtn.onclick = function(){eval(txtar2.value)};
